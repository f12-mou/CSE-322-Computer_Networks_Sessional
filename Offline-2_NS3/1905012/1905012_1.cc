...
SecondTraceFunction(std::string context, Ptr<const Packet> packet, const Address& address)
{
    ...
    // std::cout << "there " << total2 << std::endl;
}
...
main(int argc, char* argv[])
{
    ...
    uint Tx_range = 5;          // default parameter

    // Variable Parameters
    uint32_t nWifi = 100;         // 20, 40, 60, 80, 100.
    uint32_t appPacketRate = 100; // 100, 200, 300, 400, 500
    int32_t num_flow = 50;        // 10 , 20 , 40 , 70 , 100  -----> x 2
    uint Ty_range = 4;            // 1 2 3 4 5
    std::string thFile = "";
    ...
    cmd.AddValue("Ty_range", "Dimension y for coverage area", Ty_range);
    ...
    pos->Add(Vector(Tx_range, Ty_range, depth));

    pos->Add(Vector(0.1, 0.1, depth));

    for (uint32_t i = 0; i < nWifi - 2; i++)
    {
        double theta = utheta->GetValue(0, Tx_range);
        double x = theta;
        theta = utheta->GetValue(0, Ty_range);
        double y = theta;
    ...
    {
        // create sink app on right nodes
    ...
    for (int i = 0; i < nWifi; i++)
    {
        ...
        Config::Connect(oss.str(), MakeCallback(&SecondTraceFunction2));
    }*/
